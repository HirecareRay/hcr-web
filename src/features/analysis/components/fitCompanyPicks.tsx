 ...
 * fitCompanyPicks.tsx
 *
 * 적합도 분석 진입 화면의 "인기 기업" 섹션입니다. interviewCompanyPicks.tsx와 동일하게
 * 홈 피드의 트렌딩 기업을 재사용해 바로가기 칩으로 보여주지만, 눌렀을 때 면접이 아니라
 * 그 기업의 채용공고를 보여주는 검색 결과로 이동한다.
 *   → /search?q=[회사명]&tab=job
 */
...
export function FitCompanyPicks() {
  ...
  // 에러/빈 목록이면 섹션을 숨겨 진입 흐름을 막지 않는다(주 경로는 검색 버튼).
...
function SectionTitle() {
  return (
    <div className="flex items-baseline justify-between">
      <h2 className="text-ink text-sm font-bold">인기 기업</h2>
      <span className="text-disabled text-xs">눌러서 채용공고 보기</span>
    </div>
  )
}
...
function CompanyPickCard({ company }: { company: TrendingCompany }) {
  return (
    <Link
      href={routes.searchWithKeyword(company.name, "job")}
