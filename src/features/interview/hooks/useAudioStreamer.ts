 ...
 *
 * 배치 STT(useStt)와는 책임이 분리됩니다:
 *   - useStt        : 답변 전체를 녹음 후 멈추면 REST 로 전사(채점 텍스트의 진실원)
 *   - useAudioStreamer : 같은 스트림을 실시간으로 잘라 WS 로 송신(실시간 전사/평가용 "그림자")
 * 한 MediaStream 에 MediaRecorder 를 여러 개 붙이는 것은 허용되므로 둘은 공존합니다.
 *
