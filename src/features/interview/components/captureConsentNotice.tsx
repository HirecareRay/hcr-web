 ...
 *
 * 비언어 분석용 카메라 사용·스냅샷 전송에 대한 프라이버시 고지/동의 카드입니다.
 * 답변 중 얼굴을 분석하고, 시선이탈·무표정 등 "이벤트 순간"에만 화면 한 장을 분석 서버로
 * 전송한다는 점을 고지하고 명시적 동의를 받습니다. (원본 영상 스트림은 전송하지 않음)
 *
    ...
    <div className="border-warm-border rounded-2xl border bg-white p-6 shadow-sm">
      ...
      <ul className="text-muted mt-4 space-y-2 text-sm">
        ...
        <li className="flex gap-2">
          <span className="text-primary mt-px font-bold">·</span>
          <span>
            시선 이탈·무표정이 일정 시간 지속되는 등{" "}
            <span className="text-ink font-semibold">이벤트 순간에만</span> 화면을 한 장 캡처해 함께
            전송합니다(피드백 근거용).
          </span>
        </li>
      </ul>
      ...
      <label className="border-warm-border bg-warm-bg mt-4 flex cursor-pointer items-start gap-3 rounded-xl border px-3.5 py-3">
        ...
        <span className="text-ink text-sm">
          위 내용을 확인했으며, 카메라 사용과 이벤트 스냅샷 전송에 동의합니다.
        </span>
      </label>
    </div>
  )
